...
import BlogListViewCart from './../../component/carts/bloglistview';

const {height, width} = Dimensions.get('window');

const BlogListView = ({navigation}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{marginVertical: height * 0.022}}>
          <FlatList
            data={data.bloglistviewtype}
            renderItem={({item, index}) => (
              <BlogListViewCart
                img={item?.img}
                title={item?.title}
